package dev.gravy.copyingtable.menu;

import dev.gravy.copyingtable.*;
...
public class CopyingTableMenu extends AbstractContainerMenu {
    /*
    * Slot Definitions
    ...
    */
    ...
    public final Container inputSlots = new SimpleContainer(3) {
        ...
        public void setChanged() {
            CopyingTableMenu.this.slotsChanged(this);
            super.setChanged();
        }
    };
    ...
    public CopyingTableMenu(int syncId, Inventory playerInventory) {
        this(syncId, playerInventory, ContainerLevelAccess.NULL);
    }

    private CopyingTableMenu(int syncId, Inventory playerInventory, ContainerLevelAccess containerLevelAccess) {
        super(
                ModMenuTypes.COPYING_TABLE,
                syncId
        );
  ...
  this.addSlot(new Slot(this.inputSlots, MATERIAL_SLOT, 15, 51) {
        ...
        this.addSlot(new Slot(this.resultSlots, RESULT_SLOT, 145, 39) {
            ...
            public void onTake(@NonNull Player player, @NonNull ItemStack itemStack) {
                if (!player.hasInfiniteMaterials()) {
                    player.giveExperienceLevels(-CopyingTableMenu.this.cost.get());
                }

                CopyingTableMenu.this.cost.set(0);
                CopyingTableMenu.this.resultSlots.awardUsedRecipes(player, CopyingTableMenu.this.getRelevantItems());
                CopyingTableMenu.this.shrinkStackInSlot(ENCHANTED_BOOK_SLOT, ENCHANTED_BOOK_REQUIREMENT);
                CopyingTableMenu.this.shrinkStackInSlot(BOOK_SLOT, BOOK_REQUIREMENT);
                CopyingTableMenu.this.shrinkStackInSlot(MATERIAL_SLOT, MATERIAL_SLOT);
            ...
            public boolean mayPickup(@NonNull Player player) {
                return (player.hasInfiniteMaterials()
                        || player.experienceLevel >= CopyingTableMenu.this.cost.get())
                        && CopyingTableMenu.this.cost.get() > 0;
            }
        });
    ...
    public boolean stillValid(@NonNull Player player) {
        return stillValid(this.access, player, ModBlocks.COPYING_TABLE);
    }
